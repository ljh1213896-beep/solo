...
export default function ProjectCursor({ variant }:{ variant:'flamingo'|'anchor' }) {
  ...
  useEffect(() => {
    ...
    const move = (event: PointerEvent) => {
      ...
      lastX = event.clientX;
      lastY = event.clientY;
      const count = variant === 'flamingo' ? Math.min(3, 1 + Math.floor(distance / 34)) : Math.min(4, 1 + Math.floor(distance / 24));
      for (let index = 0; index < count; index += 1) particles.push({
        ...
        life:1,
        size:variant === 'flamingo' ? 1.8 + Math.random() * 1.5 : .55 + Math.random() * 1.25,
    ...
    const draw = () => {
      ...
      for (const particle of particles) {
        ...
        particle.life *= variant === 'flamingo' ? .954 : .947;
        context.save();
  ...
  return <>
    ...
    <span ref={cursorRef} className={`project-cursor-icon is-${variant}`} aria-hidden="true">{variant === 'flamingo' ? '🦩' : '⚓'}</span>
  </>;
}
